...
shinyServer(function(input, output) {
  ...
  output$downloadID <- downloadHandler(filename = "CandidateID.csv",
                                       content  = function(file){
                                         data = read.csv("data/CandidateID.csv")
                                         write.csv(data, file)
                                       })
  ...
  output$clustermap <- renderPlot({
    input$draw
    login = input$draw
    if(input$methods == "median"){
      load("data/corrmat_median.Rda")
    }else{
      load("data/corrmat_sum.Rda")
    }
    if(input$color == "GreenRed"){
      color = colorRampPalette(c("green", "black","red"))(512)
    }else{
      color = colorRampPalette(c("blue", "white","red"))(512)
    }
    if(!is.null(login)){
      names = user()
      tcgaID <- as.data.frame(select(filter(session_db, username == names), tcgaID))[1,]
      tcgaID <- str_split(tcgaID, pattern = ",")[[1]]
      heatmap3(mat[tcgaID,,drop=FALSE], col = color, cexCol = 0.0001)
    }
